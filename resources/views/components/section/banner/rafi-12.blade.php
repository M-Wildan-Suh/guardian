<div class="w-screen relative">
  <div class="w-full h-[calc(50vh-40px)] sm:h-[calc(100vh-80px)]">
        <div class="swiper heroSwiper w-full h-full relative">
            <div class="swiper-wrapper">
                ...
                <div class="swiper-slide relative w-full h-full">
                    ...
                    <div class="absolute inset-x-0 bottom-0 z-10 px-6 pb-8 sm:pb-14">
                        <div class="backdrop-blur-md bg-main/10 border border-main/30 rounded-xl p-6 text-white max-w-3xl mx-auto shadow-lg animate-fadeInUp">
                            <div class="flex flex-wrap gap-2 justify-center mb-4">
                                @foreach ($item->articles->articlecategory as $category)
                                <a href="{{ route('category', ['category' => $category->slug]) }}"
                                    class="text-xs px-3 py-1 border border-main/40 rounded-full flex items-center gap-1 backdrop-blur-sm bg-main/20 hover:bg-main/30 transition">
                            ...
                            <a href="{{ route('detail', ['slug' => $item->slug]) }}">
                                <h2 class="text-2xl sm:text-3xl font-bold leading-tight text-center line-clamp-2 hover:text-main transition">
                                    {{ $item->judul }}
                                </h2>
                            </a>
                            ...
                            <p class="text-xs sm:text-sm font-light mt-4 text-center italic">
                                <a href="{{ route('author', ['username' => $item->articles->user->slug]) }}" class="font-semibold text-main hover:underline">
            ...
            <div class="absolute bottom-0 left-0 w-full h-1 bg-main/20">
                <div class="autoplay-progress-bar h-full bg-main transition-all duration-500 ease-linear w-0"></div>
            </div>
        </div>
    </div>
</div>

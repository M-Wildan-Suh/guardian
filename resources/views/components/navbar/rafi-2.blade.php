<div class="sticky top-0 z-40" x-data="{ open: false, searchOpen: false }">
  <div class="bg-white shadow-md shadow-black/10">
    <div class="max-w-[1080px] mx-auto flex items-center justify-between px-4 md:px-8 py-4">
      ...
      {{-- Desktop --}}
      <nav class="hidden md:flex gap-6 items-center text-neutral-500 transition-all duration-300">
        <x-navbar.button.type-a title="Beranda" :route="route('home')"/>
        ...
        <div x-data="{ article: false }" class="relative">
          ...
          <div x-show="article" 
            @click.away="article = false"
            x-transition
            class="absolute top-full left-0 bg-white py-2 rounded-md shadow-md shadow-black/20 text-sm z-20 max-h-36 overflow-auto flex flex-col gap-1">
            <a href="{{ route('article') }}" class="w-full px-4 hover:bg-neutral-100 duration-300 py-1">Artikel Terbaru</a>
            @foreach ($category as $item)
            <a href="{{ route('category', ['category' => $item->slug]) }}" class="w-full px-4 hover:bg-neutral-200 duration-300 py-1">{{ $item->category }}</a>
            @endforeach
          </div>
        </div>

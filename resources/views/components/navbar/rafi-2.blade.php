...
<div class="sticky top-0 z-40 bg-white shadow-md" x-data="{ open: false, article: false }">
  ...
  {{-- Beranda --}}
  <a href="{{ route('home') }}"
     class="{{ request()->routeIs('home') ? 'text-second underline' : 'text-black hover:text-black hover:underline' }} text-xl font-semibold duration-300">
     Beranda
  </a>

  {{-- Artikel --}}
  <div x-data="{ articleOpen: false }" class="relative text-xl font-semibold">
    <a href="{{ route('article') }}"
       @click.prevent="articleOpen = !articleOpen"
       class="{{ request()->routeIs('article*') || request()->routeIs('category*') || request()->routeIs('tag*') || request()->routeIs('detail') ? 'text-second underline' : 'text-black hover:text-second hover:underline' }} cursor-pointer duration-300">
       Artikel
    </a>
    ...
    <div x-show="articleOpen" @click.outside="articleOpen = false"
      ...
      <div class="flex flex-col text-sm text-black">
        <a href="{{ route('article') }}" class="px-4 py-2 hover:bg-neutral-100 duration-300">Artikel Terbaru</a>
        @foreach ($category as $item)
          <a href="{{ route('category', ['category' => $item->slug]) }}" class="px-4 py-2 hover:bg-neutral-100 duration-300">
            {{ $item->category }}
          </a>
        @endforeach
      </div>
  ...
  <a href="{{ request()->routeIs('detail') ? route('home') : '' }}#kontak"
     class="{{ request()->is('*#kontak') ? 'text-second underline' : 'text-black hover:text-second hover:underline' }} text-xl font-semibold duration-300">
     Kontak
  </a>
</div>
  ...
  <a href="{{ route('home') }}" class="flex items-center space-x-2">
    ...
    <div x-show="open" @click.outside="open = false"
      ...
      <x-navbar.button.rafi-2 title="Beranda" :route="route('home')" :active="['home']" :mobile="true" />
      ...
      <x-navbar.button.rafi-2 title="Kontak" :route="null" :active="null" :mobile="true" />

<div x-data="{ open: false, searchOpen: false }" class="relative">
    <div
        class="fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-6 sm:px-16 lg:px-24 py-4 bg-white shadow-md">
        {{-- Logo --}}
    ...
    <div x-show="open" x-transition.opacity
        class="fixed inset-0 z-50 bg-black/80 flex flex-col items-center justify-center text-center">
        ...
        {{-- Menu --}}
        <nav class="space-y-4 bg-white/10 border border-white/20 backdrop-blur-md p-4 rounded-xl">
            <a href="{{ route('home') }}"
                class="block text-lg tracking-widest hover:bg-main hover:text-white transition text-white px-3 py-2 rounded-lg">
                Beranda
            </a>

            <div x-data="{ article: false }" class="relative">
                <button @click="article = !article"
                    class="{{ request()->routeIs('article', 'article.page', 'author', 'author.page', 'category', 'category.page', 'tag', 'tag.page', 'detail') ? 'text-white bg-second' : 'text-white hover:bg-main/40' }} 
                    w-full block text-lg tracking-widest duration-300 px-3 py-2 rounded-lg"
                    aria-label="Artikel">
                    Artikel
                </button>

                <div x-show="article"
                    class="max-h-36 overflow-auto py-2 flex flex-col gap-1 text-sm mt-1 bg-white/10 border border-white/20 rounded-lg backdrop-blur-md">
                    <a href="{{ route('article') }}"
                        class="w-full text-nowrap px-4 py-2 text-white hover:bg-main/40 rounded-lg duration-300">
                        Artikel Terbaru
                    </a>

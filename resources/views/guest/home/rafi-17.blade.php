        ...
        <div class="w-full max-w-[1100px] mx-auto p-4 grid grid-cols-1 md:grid-cols-4 gap-6 pb-4 mt-5">
            <div class="md:col-span-3 space-y-12 md:border-r md:border-gray-300 md:pr-6">
                  ...
                  {{ $i == 0 ? 'col-span-2 row-span-2 h-80 md:h-[500px]' : 'h-40 md:h-60' }}">
                        ...
                        group-hover:from-black/50 transition-all duration-500">
                            ...
                            <div class="absolute bottom-0 left-0 right-0 p-4 md:p-6">
                                <h3
                                    class="text-white font-bold text-lg md:text-xl line-clamp-2 group-hover:text-blue-300 transition-colors">
                                    {{ $item->judul }}
                                </h3>
                ...
                @foreach (array_slice($data, 0, 1) as $item)
                    <div
                        class="relative flex flex-col md:flex-row items-stretch rounded-3xl overflow-hidden shadow-xl bg-gradient-to-r from-white to-gray-50">
                        ...
                        <div class="flex flex-col justify-between p-8 w-full md:w-1/2">
                            <div>
                                <h2
                                    class="font-extrabold text-2xl md:text-3xl text-gray-900 leading-snug hover:text-blue-600 transition-colors">
                                    {{ $item->judul }}
                                </h2>
            ...
            <div class="md:pl-6">
                <div class="md:sticky top-24 space-y-4 sm:space-y-6">
                    <div class="flex items-center gap-3 sm:gap-5">
                        <div class="w-1 sm:w-1.5 h-7 sm:h-10 bg-second rounded-full"></div>
                        <p class="text-xl sm:text-2xl font-bold tracking-tight">Artikel Populer</p>
                    </div>
                    {{-- Article --}}
                    ...
                    <div class="grid grid-cols-2 gap-3 mt-6">
                        @foreach (collect($trend)->shuffle()->take(4) as $item)
                            <a href="{{ route('detail', ['slug' => $item->slug]) }}"
                                ...
                                <div>
                                    <p
                                        class="text-xs font-semibold text-gray-800 line-clamp-3 group-hover:text-blue-600 transition-colors">
                                        {{ $item->judul }}
                                    </p>
                                </div>
